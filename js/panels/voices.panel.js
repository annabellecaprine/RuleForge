(function (root) {
  ...
  function generateScript(cfg) {
    ...
    s += '  var msg = SBX_R.msgLower(context);\n';
    s += '  if (!msg) return;\n';
    ...
    s += '  function debugCrumb(context, vcfg, crumb){\n';
    s += '    if (!VOICES_CFG.debug) return;\n';
    s += '    SBX_R.append(context, \"character.scenario\", \"[\" + (vcfg.tag||\"VR\") + \":\" + crumb + \"]\", null, false, \" \");\n';
    s += '  }\n\n';

    s += '  function pickSubtone(subtones){\n';
    ...
    s += '  function runVoice(context, msg, vcfg){\n';
    ...
    s += '      SBX_R.append(context, \"character.personality\", vcfg.baselineRail, null, false, \" \");\n';
    s += '      debugCrumb(context, vcfg, \"BASE\");\n';
    s += '    }\n\n';
    s += '    if (vcfg.cadenceRail) SBX_R.append(context, \"character.personality\", vcfg.cadenceRail, null, false, \" \");\n\n';
    s += '    var ctx = vcfg.ctx || {};\n';
    s += '    var isSoft = ctx.softPhrases && ctx.softPhrases.length ? SBX_R.hasAnyPhrase(msg, ctx.softPhrases) : false;\n';
    s += '    var isTeach = ctx.teachingPhrases && ctx.teachingPhrases.length ? SBX_R.hasAnyPhrase(msg, ctx.teachingPhrases) : false;\n';
    s += '    var isComp = ctx.complimentPhrases && ctx.complimentPhrases.length ? SBX_R.hasAnyPhrase(msg, ctx.complimentPhrases) : false;\n';
    s += '    var isCont = ctx.contentWords && ctx.contentWords.length ? (SBX_R.hasAnyPhrase(msg, ctx.contentWords) || SBX_R.hasAnyWord(msg, ctx.contentWords)) : false;\n\n';
    s += '    if (isSoft) debugCrumb(context, vcfg, \"SOFT\");\n';
    s += '    if (isTeach) debugCrumb(context, vcfg, \"TEACH\");\n';
    s += '    if (isComp) debugCrumb(context, vcfg, \"COMPL\");\n';
    s += '    if (isCont) debugCrumb(context, vcfg, \"CONTENT\");\n\n';
    ...
    s += '    chance = SBX_R.clamp(chance, 0.10, 0.95);\n';
    s += '    var will = (Math.random() < chance);\n';
    s += '    debugCrumb(context, vcfg, will ? \"ATT:Y\" : \"ATT:N\");\n';
    s += '    if (!will) return;\n\n';
    s += '    var st = pickSubtone(vcfg.subtones);\n';
    s += '    if (st && st.rail) {\n';
    s += '      debugCrumb(context, vcfg, \"ST:\" + (st.label || \"?\"));\n';
    s += '      SBX_R.append(context, \"character.personality\", st.rail, null, false, \" \");\n';
    s += '    }\n';
    s += '  }\n\n';

    s += '  var voices = VOICES_CFG.voices || [];\n';
    s += '  for (var i=0;i<voices.length;i++){\n';
    s += '    if (voices[i] && voices[i].enabled) runVoice(context, msg, voices[i]);\n';
    s += '  }\n';
    s += '})();\n';
