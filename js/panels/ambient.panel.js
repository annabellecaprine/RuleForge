...
(function (root) {
  ...
  function emitES5FromAmbientRuleSpecs(rules) {
    ...
    return ''
      ...
      + "    var g = groups[gi];\n"
      + "    if(!g) continue;\n"
      + "    if(Math.random()*100 >= g.pct) continue;\n"
      + "    if(!g.items || !g.items.length) continue;\n"
      + "    var picked = pickWeighted(g.items);\n"
      + "    if(!picked || !picked.text) continue;\n"
      + "    var cur = get(context, 'context.character.scenario') || '';\n"
      + "    cur = String(cur);\n"
      + "    if(cur && cur.length && cur.charAt(cur.length-1) !== '\\n') cur += '\\n';\n"
      + "    cur += picked.text + '\\n';\n"
      + "    set(context, 'context.character.scenario', cur);\n"
      + "  }\n"
      + "})();\n";
  }
